  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h2>Willkommen bei Never Eat Alone</h2>
        <p>Melde dich an, um Dinner zu erstellen und teilzunehmen.</p>
        <label className="login-privacy-checkbox">
          <input type="checkbox" checked={accepted} onChange={(e) => setAccepted(e.target.checked)} />
          <span>
            Ich habe die <Link to="/datenschutz" onClick={onClose} className="login-privacy-link">Datenschutzerklärung</Link> gelesen und akzeptiere sie.
          </span>
        </label>
        ...
        <button className="modal-close" onClick={onClose}>Abbrechen</button>
      </div>
    </div>
  )
}
