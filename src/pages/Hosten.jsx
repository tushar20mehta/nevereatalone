...
export default function Hosten() {
  ...
  const [form, setForm] = useState({ title:'', description:'', date:'', time:'19:00', location:'', address:'', cuisine:'', maxGuests:4 })
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      await addDoc(collection(db, 'dinners'), { ...form, maxGuests: Number(form.maxGuests), hostId: user.uid, hostName: user.displayName || 'Anonym', hostPhoto: user.photoURL || '', guests: [], createdAt: serverTimestamp() })
      navigate('/my-dinners')
    } catch (error) { console.error('Fehler:', error); alert('Fehler beim Erstellen.') }
  ...
  return (
    <div className="create-page">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-primary form-submit" disabled={submitting}>{submitting ? 'Wird erstellt...' : 'Dinner veröffentlichen'}</button>
      </form>
